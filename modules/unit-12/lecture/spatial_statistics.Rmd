...
![](images/437-mapping-projection-types.png)
...
corn_yield = st_read("data/merriweather_yield_map/merriweather_yield_map.shp", quiet=TRUE)
```
...
```{r}
library(concaveman)
field_poly = concaveman(corn_yield)
```

The SSURGO data is what is in a database.  A database is a series of tables, all describing different aspects of a data information.  Each table contains 1-3 columns that are keys to match the tables with each other.  Descriptions of the tables an their data can be obtained for SSURGO at:
...
```{r}
library(FedData)
soil_map = get_ssurgo(corn_yield, "example")
