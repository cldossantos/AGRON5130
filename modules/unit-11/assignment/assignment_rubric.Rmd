...
```{r}
model1 <- lm(yield ~ rain6 + rain7 + temp6 + temp7,
             data = thompson_soy)
```
...
```{r}
plot(thompson_soy, pch = 20, cex = 0.3)
```
...
```{r}
modelr <- lm(yield ~ rain6 + rain7 + temp6,
             data = thompson_soy)
```
...
```{r}
plot(tomato$fruit_volume, tomato$days)
```
...
```{r}
model2 <- nls(fruit_volume ~ SSlogis(days, Asym, xmid, scal), 
              data = tomato)
summary(model2)
```
...
```{r}
model3 <- lm(fruit_volume ~ days + I(days^2), data = tomato)
summary(model3)
```
