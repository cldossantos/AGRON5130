...
prohibitive to the work.  If have done field work
...
hist(res, 
     main = 'Histrogram of the sample mean',
     xlab = 'Sample mean')
```
...
comparison to the standard normal distribution for
different sample sizes, and then we can think
about the effect of sample size.
...
Great! We got a value of t-value that we can use
...
negligible. Therefore, you could tell your boss to
...
Now, let's see if we had collected less samples.
Would we fail to cash in the bonus? Let's bring
the sample size to 3. If you and your boss had
agreed on a 95% threshold, you'd fail to show
evidence that the yield was different from 12
t/ha.
...
shadeDist(xshade = c(-3.42, 3.42),
          ddist = 'dt',
          parm1 = 2) ## degress of freedom: n - 1
```
...
interval (CI) cal be calculated using the formula
