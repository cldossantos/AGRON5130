...
Now we can run our contrast of the average of Bob and Donna versus the rest of the cultivars.
```{r}
library(multcomp)
...
Whoops! n_source_timing is a character variable, not a factor variable. Luckily, we now know how to fix this:

```{r}
n_source_timing$trt = as.factor(n_source_timing$trt)
class(n_source_timing$trt)
```


Now we can see the seven levels of nitrogen treatment:
